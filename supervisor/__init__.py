...
#   They represent a live socket between the supervisor and the manager.
...
class Host:
    ...
    def create_process(
        ...
        return self._context.create_process_group([self], args=args, env=env, cwd=cwd, name=name, simulate=simulate)[0]
...
class Process:
    ...
    def _response(self, msg: bytes) -> None:
        unpickled: NamedTuple = pickle.loads(msg)
...
class Filter:
    def __init__(self, context: FilteredMessageQueue, fn: Callable[[Letter], bool]):
        self._context = context
        self._fn = fn
        self._cursor = 0
...
class Context(FilteredMessageQueue):
    def __init__(
        ...
        self._delivered_messages = deque[List[Letter]]()
    ...
    def _produce_message(self, sender: Union[Host, Process], message: NamedTuple) -> None:
        self._delivered_messages_entry.append(Letter(sender, message))

    def _read_messages(self, timeout: Optional[float]) -> List[Letter]:
        if timeout is not None and not self._doorbell_poller.poll(timeout=int(1000*timeout)):
            return []
        self._doorbell.recv()
...
class LocalMessageQueue(FilteredMessageQueue):
    def __init__(self, supervisor_ident: int, supervisor_pipe: str):
        super().__init__()
...
#   get a message from any of the incomming sockets. zmq.ROUTER receives a message from one of its
